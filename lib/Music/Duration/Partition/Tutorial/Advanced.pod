...
Triplet durations will be twice as likely to be chosen, and be played
in groups of 3.
...
Play a randomized bass-line of 2 motifs, for 3 beats and rest for the
4th.
  ...
  for my $i (1 .. 8) {
      if ($i % 2) {
          $mdp->add_to_score($score, $motifs[0], \@notes1);
      }
      else {
          my @notes2 = map { $voice->rand } $motifs[1]->@*;
          $mdp->add_to_score($score, $motifs[1], \@notes2);
      }
